...
internal sealed class Publisher
{
    ...
    )
    {
        ...
        // Determine URL and build configuration
        var targetUrl = "https://www.fruva.de";
        var buildConfiguration = "Release";

        if (normalizedConfig == VALID_DEV)
        {
            targetUrl = "https://dev.fruva.de";
        ...
        // Open default browser to the target URL
        TryStartUrl(targetUrl);
    }
    ...
    private static void TryStartUrl(string url)
    {
        ...
        catch (Exception ex)
        {
            Console.Error.WriteLine(
                $"WARNING: Unable to open browser for URL '{url}': {ex.Message}"
            );
        }
    }
}
